...
/**
 * Calculates geodesic distance between two latitude/longitude points in miles
 */
...
export default function App() {
  ...
  const [viewMode, setViewMode] = useState<'grid' | 'map' | 'table' | 'analytics'>('grid');
  ...
  // Local storage persistence for saved permits watchlist
  const [savedPermitIds, setSavedPermitIds] = useState<number[]>(() => {
    try {
      const raw = localStorage.getItem('denver_demo_watchlist');
  ...
  const toggleSavePermit = useCallback((permit: DemolitionPermit) => {
    const id = permit.attributes.OBJECTID;
    setSavedPermitIds((prev) => {
      const next = prev.includes(id) ? prev.filter((x) => x !== id) : [...prev, id];
      try {
        localStorage.setItem('denver_demo_watchlist', JSON.stringify(next));
      } catch (e) {
  ...
  const filteredPermits = useMemo(() => {
    ...
    return allPermits
      .filter((permit) => {
        ...
        // Watchlist filter
        if (filters.onlySaved && !savedPermitIds.includes(attributes.OBJECTID)) {
          return false;
        }
        ...
        // Contractor filter
        if (filters.contractor && attributes.CONTRACTOR_NAME !== filters.contractor) {
          return false;
        }
        ...
        if (filters.radiusFilter) {
          const pLat = geometry?.y;
          const pLng = geometry?.x;
          if (!pLat || !pLng) return false;
          const dist = calculateDistanceMiles(
            filters.radiusFilter.lat,
            filters.radiusFilter.lng,
            pLat,
            pLng
          );
          if (dist > filters.radiusFilter.radiusMiles) {
            return false;
          }
        }
  ...
  }, [allPermits, filters, savedPermitIds]);
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col font-sans selection:bg-amber-500 selection:text-slate-950">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
        ...
        <FilterBar
          ...
          savedCount={savedPermitIds.length}
          onOpenAiInsights={() => setIsAiModalOpen(true)}
          permitsForExport={filteredPermits}
        />
        ...
        {filteredPermits.length === 0 ? (
          ...
          <AnalyticsView
            ...
            onSelectContractor={(contractor) => {
              setFilters((prev) => ({ ...prev, searchQuery: contractor, contractor }));
              setViewMode('grid');
            }}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              {paginatedPermits.map((permit) => (
                <PermitCard
                  ...
                  isSaved={savedPermitIds.includes(permit.attributes.OBJECTID)}
                  onToggleSave={toggleSavePermit}
                />
              ))}
            </div>
          ...
          <MapView
            ...
            onRadiusChange={(rf) => setFilters((prev) => ({ ...prev, radiusFilter: rf }))}
          />
        ) : (
          <TableView
            permits={filteredPermits}
      ...
      {selectedPermit && (
        <PermitDetailModal
          ...
          isSaved={savedPermitIds.includes(selectedPermit.attributes.OBJECTID)}
          onToggleSave={toggleSavePermit}
        />
      )}
      ...
      {/* AI Demolition Intelligence Modal */}
      <AiInsightsModal
      ...
      <footer className="bg-slate-950 border-t border-slate-800/80 text-slate-500 py-6 text-xs text-center">
        <div className="max-w-7xl mx-auto px-4 space-y-1">
          ...
          <p className="text-[11px] text-slate-600">
            FOSS Street View & Maps powered by OpenStreetMap, Mapillary, and CARTO. Full CSV & GeoJSON Export.
          </p>
        </div>
      </footer>
    </div>
