...
export const PermitCard: React.FC<PermitCardProps> = ({
  ...
  return (
    <div
      ...
      className="group bg-slate-900 border border-slate-800 hover:border-amber-500/50 rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-all duration-200 flex flex-col relative"
    >
      {/* Bookmark Star Button (floating top-right) */}
      {onToggleSave && (
        <button
          onClick={(e) => {
            e.stopPropagation();
          ...
          className={`absolute top-2 right-2 z-20 p-1.5 rounded-lg backdrop-blur-md transition cursor-pointer shadow-md ${
            isSaved
              ? 'bg-amber-500 text-slate-950 font-bold border border-amber-400'
              : 'bg-slate-900/80 text-slate-300 hover:text-amber-400 hover:bg-slate-900 border border-slate-700/80'
          }`}
